...
public class EventService {
  ...
  /**
   * Creates a new event from a creation request. The event ID is automatically generated. After
   * validating organization exists, user exists (if provided), and user is a member of the
   * organization.
   *
  ...
  public Event createEvent(EventCreationRequest request) {
    logger.info("Creating new event: " + (request != null ? request.getTitle() : "null"));
    if (request == null) {
      throw new IllegalArgumentException("Event creation request cannot be null");
    }

    // Convert DTO to Entity (ID will be auto-generated by Event constructor)
    ...
    if (savedEvent.getCreatedBy() != null && !savedEvent.getCreatedBy().trim().isEmpty()) {
      try {
        ...
        logger.warning("Failed to automatically add event creator as attendee: " + e.getMessage());
      }
    }
  ...
  /**
   * Updates an existing event from an update request. The event ID, creation timestamp, and creator
   * are preserved. After validating organization exists.
   *
  ...
  public Event updateEvent(String eventId, EventUpdateRequest request) {
    ...
    if (request == null) {
      throw new IllegalArgumentException("Event update request cannot be null");
    }
    ...
    // Convert DTO to Entity for validation (preserve ID, createdAt, createdBy)
    Event eventForValidation = new Event();
    eventForValidation.setTitle(request.getTitle());
    eventForValidation.setDescription(request.getDescription());
    eventForValidation.setStartAt(request.getStartAt());
    eventForValidation.setEndAt(request.getEndAt());
    eventForValidation.setCapacity(request.getCapacity());
    eventForValidation.setOrgId(request.getOrgId());
    eventForValidation.setCreatedBy(existingEvent.getCreatedBy()); // Preserve original creator

    validateEvent(eventForValidation);
    validateOrganizationExists(request.getOrgId());

    // Update the existing event with new data (preserve ID, createdAt, createdBy)
    existingEvent.setTitle(request.getTitle());
    existingEvent.setDescription(request.getDescription());
    existingEvent.setStartAt(request.getStartAt());
    existingEvent.setEndAt(request.getEndAt());
    existingEvent.setCapacity(request.getCapacity());
    existingEvent.setOrgId(request.getOrgId());
    // Note: createdBy is NOT updated - it remains the original creator
